...
export const formatIngredientItem = (item) => {
    ...
    const finalPluralName = pluralName ? pluralName : (nameNotEndWithS ? item.name + 's' : item.name);
    const name = unitlessPlural ? finalPluralName : item.name;
    ...
    if (unit.trim() == 'small' || unit.trim() == 'medium' || unit.trim() == 'large') {
        return amount + unit + (isPlural ? finalPluralName : item.name);
    }
